	...
	private void initialise() {
		...
		searchField.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int index1 = new KMP(pattern, text).search();
				long endTime1 = System.nanoTime();
				long timeTaken1 = endTime1 - startTime1;
				System.out.println("Time taken for KMP search is: " + timeTaken1 + " ns");

				long startTime2 = System.nanoTime();
				int index2 = new BruteForceSearch(pattern, text).search();
				long endTime2 = System.nanoTime();
				long timeTaken2 = endTime2 - startTime2;
				System.out.println("Time taken for brute force search is: " + timeTaken2 + " ns");

				if (index1 == -1) {
					JOptionPane.showMessageDialog(frame, "Pattern not found.");
				} else {
					textEditor.requestFocus();
					textEditor.setSelectionStart(index1);
					textEditor.setSelectionEnd(index1 + pattern.length());
